...
public class OktaAppAuth extends AppCompatActivity {
    ...
    public void fetchUserInfo(View view) {
        if (mAuthState != null) {
            if (mAuthState.getNeedsTokenRefresh()){
                createAlert("Error", "Needs token refresh");
            } else {
    ...
    /**
     * Refreshes tokens through performActionWithFreshTokens
     */
    public void callTokenEndpoint(){
        mAuthState.performActionWithFreshTokens(mAuthService, new AuthState.AuthStateAction() {
            @Override
            public void execute(String accessToken, String idToken, AuthorizationException ex) {
                if (ex != null) {
                    Log.d(TAG, "Error fetching fresh tokens");
                    createAlert("Error", "Error fetching fresh tokens");
                } else {
                    createAlert("Success", "All tokens were refreshed");
